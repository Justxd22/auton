...
import { generatePaymentRequest, verifyPayment, calculateAmounts, getFeeInfo } from '../utils/payment.js';
...
// GET /creator/by-username/:username/content - Get content by creator username
router.get('/creator/by-username/:username/content', (req, res) => {
  try {
    ...
    const contentList = database.listContent({ creatorId: creator.id }).map(sanitizeContent);
...
// GET /creator/resolve/:identifier/content - Resolve by username or wallet and get content
router.get('/creator/resolve/:identifier/content', (req, res) => {
  try {
...
router.get('/content/:contentId/paywall', (req, res) => {
  ...
  // x402-compatible headers (per x402.org spec)
  const x402Headers = {
    ...
    'X-Payment-Asset': content.assetType,
  ...
  res
    .status(402) // HTTP 402 Payment Required (x402 protocol)
    .set(x402Headers)
    .json({
      ...
      // x402-compatible response format
      x402: {
...
// GET /fees - Get platform fee information
router.get('/fees', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get fee info', details: error.message });
  }
});

// GET /fees/calculate - Calculate fee breakdown for a specific amount
router.get('/fees/calculate', (req, res) => {
  try {
    ...
    const breakdown = calculateAmounts(numericAmount);
    ...
    res.status(500).json({ error: 'Failed to calculate fees', details: error.message });
  }
});
